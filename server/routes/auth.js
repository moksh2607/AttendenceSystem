...
router.post('/signup', async (req, res) => {
  try {
    ...
    const user = new User({ name, email, passwordHash: hash, location });
    await user.save();
    const token = jwt.sign({ id: user._id, email: user.email }, JWT_SECRET, { expiresIn: '12h' });
    res.json({ success: true, token, user: { id: user._id, name: user.name, email: user.email, location: user.location } });
  } catch (err) {
    console.error(err);
